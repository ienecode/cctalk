...
CctalkDevice::CctalkDevice()
{
    ...
    qDebug() << "ThreadID CctalkDevice:  " << QThread::currentThreadId();
...
{
    qDebug() << "ThreadID in startTimer()  " << QThread::currentThreadId();

    //ponizsze powoduje ze funkcja startTimer wraca do pierwotnego wątku :/
...
{
    qDebug() << "ThreadID in stopTimer()  " << QThread::currentThreadId();
